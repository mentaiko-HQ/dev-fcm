 ...
 * - props の participants をオプショナル（participants?: Participant[]）とし、未指定時は空配列 [] を既定値として代入。
 * - 未使用インポート（ProgressStatus, QualificationStatus, sanitizeCheckInStatus）を型アノテーションおよびサニタイズ処理に実質バインドし警告を根絶。
 * - 選手氏名が空白のまま保存されることを防止する必須入力検証。
 * - 更新処理中（updatingIds, isSaving）のボタン非活性化により多重実行・連打を完全抑止。
 * - 型ガード（sanitizeRankTitleType, sanitizeCheckInStatus, sanitizeProgressStatus, sanitizeQualificationStatus）を通して不正値の混入を遮断。
 *
 * フェイルセーフ設計（障害発生時の安全縮退）:
 * - React purity ルール遵守: `Date.now()` のレンダー内直接実行を排除し、コンポーネント外ヘルパー `getSafeTimestamp()` を経由してイベントハンドラ内で実行。
 * - useMemo 内で Array.isArray(participants) を検証し、親コンポーネントから undefined や null、
 *   非配列データが渡された場合でも空配列へフォールバックして TypeError（クラッシュ）を防止。
 * - Firestore 通信エラー発生時でも画面を落とさず、エラーメッセージをUI上に表示してログを記録。
 */
...
/**
 * 【フェイルセーフ】レンダー本体から不純な呼び出し（Date.now）を分離する外部ヘルパー関数
 */
...
export function ParticipantDataTable({
  ...
  const [filterCheckIn, setFilterCheckIn] = useState<CheckInStatus | 'ALL'>(
    'ALL',
  );
  const [filterProgress, setFilterProgress] = useState<ProgressStatus | 'ALL'>(
    'ALL',
  );
  ...
  const [editForm, setEditForm] = useState<{
    ...
    standAssignments: Record<
      StandRoundIndex,
      { standGroup: number; standOrder: StandOrderIndex }
    >;
    notes: string;
  }>({
    bibNumber: 1,
  ...
  // 【フェイルセーフ】安全な配列検証とフィルタリング・ソートの算出
    ...
    return safeList
      .filter((p) => {
        ...
        const matchesQuery =
          ...
          (p.nameKana && p.nameKana.toLowerCase().includes(queryLower)) ||
          (p.organization &&
        ...
        if (filterCheckIn !== 'ALL' && p.checkInStatus !== filterCheckIn)
          return false;
        if (filterProgress !== 'ALL' && p.progressStatus !== filterProgress)
          return false;
        if (
          filterQualification !== 'ALL' &&
          p.qualificationStatus !== filterQualification
        )
          return false;
      ...
      .sort((a, b) => {
        ...
        switch (sortField) {
          case 'bibNumber':
            diff = (a.bibNumber ?? 0) - (b.bibNumber ?? 0);
            break;
          case 'name':
            diff = (a.nameKana || a.name || '').localeCompare(
              b.nameKana || b.name || '',
              'ja',
            );
            break;
          case 'organization':
            diff = (a.organization || '').localeCompare(
              b.organization || '',
              'ja',
            );
            break;
          case 'stand1': {
            const a1 = a.standAssignments?.[1] || {
              standGroup: 999,
              standOrder: 999,
            };
            const b1 = b.standAssignments?.[1] || {
              standGroup: 999,
              standOrder: 999,
            };
            diff =
              a1.standGroup !== b1.standGroup
                ? a1.standGroup - b1.standGroup
                : a1.standOrder - b1.standOrder;
            break;
          }
          case 'stand2': {
            const a2 = a.standAssignments?.[2] || {
              standGroup: 999,
              standOrder: 999,
            };
            const b2 = b.standAssignments?.[2] || {
              standGroup: 999,
              standOrder: 999,
            };
            diff =
              a2.standGroup !== b2.standGroup
                ? a2.standGroup - b2.standGroup
                : a2.standOrder - b2.standOrder;
            break;
          }
          case 'checkInStatus':
            ...
            break;
          case 'progressStatus':
            diff = (a.progressStatus || '').localeCompare(
              b.progressStatus || '',
            );
            break;
          case 'qualificationStatus':
            diff = (a.qualificationStatus || '').localeCompare(
  ...
  // Firestore フィールド更新共通処理
  const updateParticipantField = useCallback(
    async (participantId: string, fields: Partial<Participant>) => {
  ...
  // 受付ステータス切り替えハンドラー（不純な呼び出しを外部ヘルパー経由で実行）
  const handleToggleCheckIn = useCallback(
    async (p: Participant) => {
      const nextStatus: CheckInStatus =
        p.checkInStatus === 'UNCHECKED'
          ? 'CHECKED_IN'
          : p.checkInStatus === 'CHECKED_IN'
            ? 'ABSENT'
            : 'UNCHECKED';
      const checkInAt = nextStatus === 'CHECKED_IN' ? getSafeTimestamp() : null;

      await updateParticipantField(p.id, {
        checkInStatus: sanitizeCheckInStatus(nextStatus),
        checkInAt,
      });
    },
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await updateParticipantField(editingParticipant.id, {
        ...
        staffDutyShift:
          editForm.staffDutyShift as Participant['staffDutyShift'],
  ...
  const totalCount = Array.isArray(participants) ? participants.length : 0;
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-sm space-y-3">
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <Badge
              variant="outline"
              className="font-mono text-xs text-slate-700 bg-slate-50 px-2.5 py-1"
            >
              該当: {processedParticipants.length} / 全 {totalCount} 名
            </Badge>
          </div>
        </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setFilterCheckIn(
                  val === 'ALL' ? 'ALL' : sanitizeCheckInStatus(val),
                );
              }}
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                const val = e.target.value;
                setFilterProgress(
                  val === 'ALL' ? 'ALL' : sanitizeProgressStatus(val),
                );
              }}
              className="w-full p-1.5 border border-slate-200 rounded-lg bg-white text-slate-800 font-medium focus:ring-1 focus:ring-slate-900 outline-none"
            >
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setFilterQualification(
                  val === 'ALL' ? 'ALL' : sanitizeQualificationStatus(val),
                );
              }}
              className="w-full p-1.5 border border-slate-200 rounded-lg bg-white text-slate-800 font-medium focus:ring-1 focus:ring-slate-900 outline-none"
            >
            ...
            <tbody className="divide-y divide-slate-100">
              {processedParticipants.length > 0 ? (
                processedParticipants.map((p) => {
                  ...
                  const a1 = p.standAssignments?.[1] || {
                    standGroup: 1,
                    standOrder: 1,
                  };
                  const a2 = p.standAssignments?.[2] || {
                    standGroup: 1,
                    standOrder: 1,
                  };
                    ...
                    >
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            updateParticipantField(p.id, {
                              progressStatus: sanitizeProgressStatus(
                                e.target.value,
                              ),
                            })
                          }
                      ...
                      <td className="p-3.5 text-center">
                        <select
                          ...
                          onChange={(e) =>
                            updateParticipantField(p.id, {
                              qualificationStatus: sanitizeQualificationStatus(
                                e.target.value,
                              ),
                            })
                          }
            ...
            <div className="space-y-4 text-xs max-h-[70vh] overflow-y-auto pr-1">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditForm((prev) => ({
                        ...prev,
                        bibNumber: Number(e.target.value),
                      }))
                    }
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditForm((prev) => ({
                        ...prev,
                        shosa: e.target.value as ShosaType,
                      }))
                    }
              ...
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
                ...
                {([1, 2] as const).map((roundIdx) => (
                  <div
                    key={roundIdx}
                    className="grid grid-cols-3 gap-2 items-center bg-white p-2.5 rounded-lg border border-slate-200"
                  >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        min={1}
                        value={
                          editForm.standAssignments[roundIdx]?.standGroup || 1
                        }
                        onChange={(e) => {
                    ...
                    <div>
                      ...
                      <select
                        value={
                          editForm.standAssignments[roundIdx]?.standOrder || 1
                        }
                        onChange={(e) => {
